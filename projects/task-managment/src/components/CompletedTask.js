...
const CompletedTask = () => {
  ...
  const completeTaskDelete = (id) => {
    ...
    if (confirm) {
      fetch(
        `https://task-management-server-red.vercel.app/completedTaskDelete/${id}`,
        {
          method: "DELETE",
        }
      )
    ...
    <div>
      <div className="overflow-x-auto relative">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {completedTaskData.map((complete) => (
              <tr
              ...
              >
                ...
                <td className="py-4 px-6">
                  <button onClick={() => completeTaskDelete(complete._id)}>
      ...
      <Comment showModal={showModal} setShowModal={setShowModal}/>
    </div>
  );
};
